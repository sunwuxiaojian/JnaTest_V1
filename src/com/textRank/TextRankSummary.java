...
//                "三，无限的算法，是那些由于没有定义终止定义条件，或定义的条件无法由输入的数据满足而不终止运行的算法。通常，无限算法的产生是由于未能确定的定义终止条件。";
        String document = FileUtil.readNewsFile("src/test1.txt");

        TextRankSummary.getTopSentenceList(document, 5).forEach(System.out::println);
    }
...
//        return CoreStopWordDictionary.shouldInclude(term);
    ...
    public static List<String> getTopSentenceList(String document, int size)
    {
        ...
        {
//            List<Term> terList = HanLP.segment(sentence);
            String result = tool.NLPIR_ParagraphProcess(sentence,0);
            List<String> resultList = Arrays.asList(result.split(" "));
//            System.out. println(result);
            List<String> wordList = new LinkedList<String>();
            Set<String> stopWordSet = FileUtil.readWordByLine("D:\\NLPIR\\stopwords.txt");
